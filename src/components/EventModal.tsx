      ...
      <div className="absolute bottom-0 left-0 right-0 max-w-lg mx-auto max-h-[88vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="bg-white rounded-t-3xl px-6 pt-3 pb-8 safe-bottom animate-slide-up">
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="flex-1 py-3.5 bg-surface-secondary text-text-primary font-medium rounded-2xl text-sm active:scale-[0.98] transition-transform"
            >
              닫기
            </button>
            <button
              onClick={() => setEditing(true)}
              className="flex-1 py-3.5 bg-futsal text-white font-semibold rounded-2xl text-sm active:scale-[0.98] transition-transform flex items-center justify-center gap-1.5"
            >
